...
@Component({
  selector: 'app-crear-ma',
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule],
  templateUrl: './crear-ma.component.html',
  styleUrls: ['./crear-ma.component.css']
})
export class CrearMaComponent implements OnInit {
  ...
  onCreate(): void {
    ...
    if (confirmCreate) {
      ...
      this.apiService.createDocente(docenteData).subscribe({
        next: (response) => {
          alert('docente creada exitosamente');
          this.isLoading = false;
          this.router.navigate(['/maestro']);
        },
        error: (error) => {
          console.error('Error al crear el docente:', error);
          alert('Ocurrió un error al crear el docente: ' + error.message);
          this.isLoading = false;
        }
      });
    }
